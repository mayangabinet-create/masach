 ...
 * מסך. — סקרייפר רב חן
 *
 * רב חן רץ על פלטפורמת ה-quickbook של Cineworld (siteId 10104), עם
 * API נקי ותקני: קולנועים → תאריכים פעילים לכל קולנוע → סרטים+הקרנות
 * לכל תאריך. נמצא ע"י מעקב אחר בקשות הרשת בדפדפן אמיתי (ראו
 * scripts/probe-network.mjs ו-data/probe.md).
 *
 * רץ אחרי scrape.mjs (סינמה סיטי) ומוסיף את עצמו לאותם קבצים בלי
 * לדרוס — קורא את data/movies.json + data/showtimes.json הקיימים,
 * מסנן החוצה רק את הרשומות של עצמו (מזהי "rh-"), וכותב מחדש.
 *
...
const SITE_ID = "10104";
const BASE = "https://www.rav-hen.co.il/rh/data-api-service/v1/quickbook";
const CHAIN = "רב חן";
...
/* attributeIds מערבבים ז'אנר/דירוג-גיל/שפה/פורמט בלי הפרדה — לוקחים
   את ההתאמה הראשונה מתוך רשימות ידועות (ראו data/probe.md — attributes) */
const GENRE_HE = {
...
async function main() {
  ...
  const cinemas = (cinemasResp.body?.cinemas || []).map(c => ({
    id: `rh-${c.id}`, rawId: c.id, chain: CHAIN, name: c.displayName,
    ...
    for (const date of dates) {
      ...
      try {
        ...
        for (const ev of fe.body?.events || []) {
          screenings.push({
            movieId: `rh-${ev.filmId}`, cinemaId: c.id,
  ...
  if (!screenings.length) {
    console.error("\nלא נמצאה אף הקרנה של רב חן — כנראה מבנה ה-API השתנה.");
    process.exit(1);
  }

  /* מיזוג: שומרים כל מה שאינו רב חן מהקבצים הקיימים, מחליפים את
     החלק של רב חן בטרי, ומצמצמים את מאגר הסרטים למה שבאמת מוקרן. */
  let existing = { cinemas: [], screenings: [] };
  try { existing = JSON.parse(await readFile("data/showtimes.json", "utf8")); } catch {}

  const keepCinemas = (existing.cinemas || []).filter(c => !c.id.startsWith("rh-"));
  const keepScreenings = (existing.screenings || []).filter(s => !s.cinemaId.startsWith("rh-"));
  ...
  console.log(`\nנשמר: ${screenings.length} הקרנות רב חן · סה"כ ${allScreenings.length} הקרנות בקובץ`);
}
